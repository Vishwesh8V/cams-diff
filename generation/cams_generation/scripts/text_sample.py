...
from cams_generation.rounding import rounding_func, load_models, load_tokenizer
...
from cams_generation.test_util import get_weights, denoised_fn_round
...
def main():
    ...
    # Determine timesteps to sample over. --custom_timesteps_path (a plain
    # .npy list of strictly-decreasing raw timestep indices) takes priority
    # over --timestep_respacing's evenly-strided range when given -- this is
    # the DDIM/p_sample_loop analogue of --step_matrix_path for the
    # token-adaptive DPM-Solver path: both let the caller specify exactly
    # which raw timesteps get visited instead of only an evenly-spaced count.
    custom_timesteps_path = getattr(args, 'custom_timesteps_path', '')
    if custom_timesteps_path:
        if not os.path.exists(custom_timesteps_path):
            raise FileNotFoundError(
                f"--custom_timesteps_path={custom_timesteps_path} does not exist."
            )
        loaded_ts = np.load(custom_timesteps_path).astype(np.int64)
        if loaded_ts.ndim != 1:
            raise ValueError(
                f"--custom_timesteps_path must be a 1-D array of raw "
                f"timesteps, got shape {loaded_ts.shape}."
            )
        if not np.all(loaded_ts[:-1] > loaded_ts[1:]):
            raise ValueError(
                f"--custom_timesteps_path values must be strictly "
                f"decreasing; got {loaded_ts.tolist()}"
            )
        if loaded_ts.min() < 0 or loaded_ts.max() >= 2000:
            raise ValueError(
                f"--custom_timesteps_path values must be in [0, 1999]; got "
                f"min={loaded_ts.min()}, max={loaded_ts.max()}."
            )
        use_timesteps = [int(t) for t in loaded_ts.tolist()]
        logger.log(
            f"### Loaded {len(use_timesteps)} custom timesteps from "
            f"{custom_timesteps_path}: {use_timesteps}"
        )
    elif args.timestep_respacing and args.timestep_respacing.isdigit():
...
def create_argparser():
    defaults = dict(
        ...
        verbose='yes',
        out_dir="diffusion_lm/cams_generation/out_gen"
    )
