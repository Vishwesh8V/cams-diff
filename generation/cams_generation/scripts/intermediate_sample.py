...
cams_generation_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, ".."))
TRANSFORMERS_SRC = os.path.abspath(os.path.join(PROJECT_ROOT, "transformers/src"))
for p in [TRANSFORMERS_SRC, cams_generation_DIR]:
...
from cams_generation.respace import SpacedDiffusion
...
class CallRecorder:
    """
    Records model calls made by the sampler, in visiting order (noisiest ->
    cleanest): each entry is (raw_diffusion_t, pred_xstart).

    IMPORTANT: pass `needed_indices` (a set of 0-indexed call positions --
    see compute_needed_call_indices) whenever total_calls could be large
    (ancestral/ddim at low stride -> up to 2000 calls). Without it, every
    single call's full-batch pred_xstart tensor gets cloned and kept alive
    for the entire run even though write_milestones only ever reads a
    handful of them -- at batch=900, seq_len=256, that's ~28MB per
    snapshot, and 2000 of them is ~55GB, which is exactly what caused a
    real CUDA OOM (crashing at step ~1974/2000 after ~3 hours, right as
    the linearly-accumulating memory crossed the GPU's ceiling -- not a
    "batch too big for one step" problem, since 1974 individual steps had
    already succeeded).

    needed_indices=None keeps the old "record everything" behavior, which
    is fine for samplers whose total_calls is small (dpm_solver/
    token_adaptive at their usual few-hundred-step budgets) but should be
    avoided for ancestral/ddim at high step counts.
    """

    def __init__(self, needed_indices=None):
        self.calls = {}            # sparse: call_idx0 -> (raw_t, x_batch)
        self.needed_indices = needed_indices
        self.total_calls_made = 0  # TRUE call count, independent of how many we kept

    def record(self, raw_t, x_batch):
        idx = self.total_calls_made
        self.total_calls_made += 1
        if self.needed_indices is None or idx in self.needed_indices:
            self.calls[idx] = (int(raw_t), x_batch.detach().clone())
        # else: deliberately not cloned/stored -- this is the memory fix.


def compute_needed_call_indices(total_calls, num_milestones, manual_labels):
    """
    Precompute exactly which 0-indexed call positions write_milestones will
    ever read, so CallRecorder can discard everything else on the fly
    instead of holding the entire run in memory. label=0 is excluded since
    it's served from the sampler's true final output, never from a
    recorded call. Uses the identical label logic write_milestones uses,
    so what gets kept is guaranteed to match what gets requested later.
    """
    ...
    return {total_calls - label for label in labels if label != 0}, labels
...
def write_milestones(outputdir, recorder, final_sample, num_milestones, model, tokenizer, answers,
    ...
    total_calls = recorder.total_calls_made
    if manual_labels is not None:
        labels = resolve_manual_labels(manual_labels, total_calls)
...
# --------------------------------------------------------------------------
def run_ancestral(diffusion, model, sample_shape, desc, args, manual_labels=None):
    total_calls = diffusion.num_timesteps  # known upfront -- no need to run the loop first
    needed_indices, labels = compute_needed_call_indices(total_calls, args.num_milestones, manual_labels)
    print(f"[Memory] recording only {len(needed_indices)} of {total_calls} pred_xstart "
          f"snapshots (labels {labels}) instead of all {total_calls} -- avoids the "
          f"OOM this would otherwise cause at large batch sizes.")
    recorder = CallRecorder(needed_indices=needed_indices)
...
# --------------------------------------------------------------------------
def run_ddim(diffusion, model, sample_shape, desc, args, manual_labels=None):
    total_calls = diffusion.num_timesteps
    needed_indices, labels = compute_needed_call_indices(total_calls, args.num_milestones, manual_labels)
    print(f"[Memory] recording only {len(needed_indices)} of {total_calls} pred_xstart "
          f"snapshots (labels {labels}) instead of all {total_calls} -- avoids the "
          f"OOM this would otherwise cause at large batch sizes.")
...
# --------------------------------------------------------------------------
def run_dpm_solver(dpm_solver_diffusion, model, sample_shape, desc, args, device, manual_labels=None):
    from cams_generation.dpm_solver import NoiseScheduleVP, DPM_Solver, expand_dims
    ...
    total_calls = args.dpm_solver_steps
    needed_indices, labels = compute_needed_call_indices(total_calls, args.num_milestones, manual_labels)
    print(f"[Memory] recording only {len(needed_indices)} of {total_calls} pred_xstart "
          f"snapshots (labels {labels}).")
    recorder = CallRecorder(needed_indices=needed_indices)
...
# --------------------------------------------------------------------------
def run_token_adaptive(token_adaptive_diffusion, model, sample_shape, desc, step_matrix, args, device, manual_labels=None):
    total_calls = step_matrix.shape[0]  # K -- known upfront from the already-loaded step_matrix
    needed_indices, labels = compute_needed_call_indices(total_calls, args.num_milestones, manual_labels)
    print(f"[Memory] recording only {len(needed_indices)} of {total_calls} pred_xstart "
          f"snapshots (labels {labels}).")
    recorder = CallRecorder(needed_indices=needed_indices)
...
def main():
    ...
    if args.sampler in ("ancestral", "ddim"):
        ...
        if args.sampler == "ancestral":
            final_sample, recorder = run_ancestral(diffusion, model, sample_shape, desc_dev, args, manual_labels=manual_labels)
        else:
            final_sample, recorder = run_ddim(diffusion, model, sample_shape, desc_dev, args, manual_labels=manual_labels)
    else:
        ...
        if args.sampler == "dpm_solver":
            final_sample, recorder = run_dpm_solver(base_diffusion, model, sample_shape, desc_dev, args, device, manual_labels=manual_labels)
        elif args.sampler == "token_adaptive":
            from cams_generation.dpm_solver import load_step_matrix
            ...
            final_sample, recorder = run_token_adaptive(base_diffusion, model, sample_shape, desc_dev,
                                                          step_matrix, args, device, manual_labels=manual_labels)
        else:
