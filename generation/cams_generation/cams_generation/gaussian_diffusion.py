...
class GaussianDiffusion:
    ...
    def ddim_sample(
        ...
        # Apply discrete absorbing substitution at inference matching
        # training -- same mechanism p_sample already has, but using
        # sqrt_one_minus_alphas_cumprod (the cumulative marginal noise
        # level) rather than a step-wise posterior std. This is the exact
        # formula q_sample uses during training, and the one the
        # token-adaptive DPM-Solver path already uses via marginal_std;
        # p_sample's own posterior-based version is a known, separate
        # not something to replicate here.
        mean_embed = getattr(model, 'mean_embed', None)
        if self.denoise and mean_embed is not None:
            mask_rate = (
                nonzero_mask[:, :, 0]
                * _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x.shape[:2])
                * self.denoise_rate
            )
            mask_rate = mask_rate.clamp(0.0, 1.0)
            random_mask = mask_rate.bernoulli()[..., None].expand(x.shape)
    ...
    def dpm_solver_sample_loop(
        ...
        """
        from .dpm_solver import NoiseScheduleVP, DPM_Solver, cams_model_wrapper
