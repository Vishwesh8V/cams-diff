...
if __name__ == '__main__':
    ...
    parser.add_argument('--seeds', type=int, nargs='+', default=None,
                    help='random seeds for inference runs (default: [123, 101, 102])')
    parser.add_argument('--seed', type=int, default=None,
                    help='single random seed (backward compatibility)')
    ...
    # Determine 3 seeds, ensuring seed 123 is included
    if args.seeds is not None:
        seeds = list(args.seeds)
    elif args.seed is not None:
        seeds = [args.seed]
        for default_s in [123, 101, 102]:
    ...
    # If the user supplied more than 3 seeds, keep only the first 3.
    seeds = seeds[:3]

    print(f"Running inference on 3 seeds: {seeds}")
    ...
    for checkpoints in checkpoint_iter:
        for checkpoint_one in checkpoints:
            ...
            # Dynamically resolve correct schedule file matching the checkpoint step
            ...
            potential_schedule = os.path.join(
                checkpoint_dir,
                f"alpha_cumprod_step_{checkpoint_step}.npy"
            )
